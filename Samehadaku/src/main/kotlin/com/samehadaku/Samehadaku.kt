...
class Samehadaku : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        when (request.name) {
            "HomePage" -> {
                if (page <= 1) {
                    ...
                    doc.select("div.widget_senction:not(:contains(Baca Komik))").forEach { block ->
                        val header =
                                block.selectFirst("div.widget-title h3")?.ownText()
                                        ?: return@forEach
            ...
            "Daftar Anime" -> {
                ...
                val home =
                        doc.select("div.relat > article.animpost").mapNotNull {
                            it.toSearchResult()
                        }
    ...
    override suspend fun loadLinks(
        ...
        // Tangani URL video dari mega.nz
        document.select("video[src^='blob:https://mega.nz/']").forEach { videoElement ->
            val megaUrl = videoElement.attr("src")
            callback.invoke(
                newExtractorLink(
                    "Mega.nz", 
                    "Mega.nz", 
        ...
        coroutineScope {
            document.select("div#downloadb li").forEach { el ->
                ...
                el.select("a").forEach { aTag ->
                    launch {
                        loadFixedExtractor(
                            fixUrl(aTag.attr("href")),
                            quality,
                            "$mainUrl/",
                            subtitleCallback,
                            callback
